...
class Selector extends Content{
     ...
     * @param idLangMap {IdLangMap} {@link Content.idLangMap}
     */
    ...
    get accordionItem(){
        ...
        <div class="accordion-item inheritColors">
            <h2 class="accordion-header inheritColors" style="display: flex; justify-content: space-between; font-size: x-large;">
                    ...
                    <div class="my-auto p-2 counter-element d-inline-flex inheritColors" style="font-size: x-large;min-width: 50px;text-wrap: auto;">
    ...
    createNextCircuitBtn(){
        ...
        nextCircuitBtn.addEventListener("click", async () => {
            ...
            this.carousels.get(selectorGroup).bootstrapCarousel.next();
    ...
    async init(){
        if (!this.circuitFiles.loaded) {
            console.log("Selector circuits not loaded yet - waiting with init until loaded");
        ...
        for (let circuitSet of this.circuitFiles.circuitSets) {
            const identifier = circuitSet.identifier
            ...
            this.carousels.set(identifier, new SelectorCarousel(body, circuitSet, this));
            let carousel = this.carousels.get(identifier);
            await carousel.init()

...
//  node := string
// need a netlist? just use the part's type, properties and connections
...
function update_schematics() {
	...
	const loadedInput = document.querySelector('input[loaded]');

	for (let i = 0; i < schematics.length; ++i)
		if (loadedInput) {
			if (schematics[i].getAttribute("loaded") == "true")
				loadedInput.setAttribute("loaded", "false");
			let nextDiv = loadedInput.nextElementSibling;
	...
	for (let i = 0; i < schematics.length; ++i)
		if (schematics[i].getAttribute("loaded") != "true") {
			try {
				window.activeSchematic = new schematic.Schematic(schematics[i]);
			} catch (err) {
				setTimeout(() => {showMessage(i18n.error1)});
...
schematic = (function () {
	...
	function Schematic(input) {
		...
		this.width = (window.innerWidth > 0) ? window.innerWidth - part_w * 2 - 20 : screen.width - part_w * 2 - 20;
		//this.canvas.width = this.width;
		this.height = input.getAttribute('height');
		//this.height = parseInt(this.height == undefined ? '300' : this.height);
		this.height = (window.innerHeight > 0) ? window.innerHeight - 145 : screen.height - 145;
		...
		this.sctl_r = 20;   				// scrolling control parameters
		...
		this.rctl_y = this.zctl_y + this.zctl_h + 12 + this.rctl_r;
		...
		if (!this.diagram_only) {
			...
			this.status_div.style.height = 'auto';
			this.status_div.style.marginLeft = '2px';
            this.status_div.style.paddingRight = '2px';
			this.status_div.style.textAlign = "left";
			this.status_div.style.font = '8pt sans-serif';
		...
		var parts_table = document.createElement('div');
		td.appendChild(parts_table);

		parts_table.style.display = 'grid';
		parts_table.style.gap = '3px';
		parts_table.style.alignContent = 'start';

		// fill in parts_table
		// var parts_per_column = Math.floor(this.height / (part_h + 5));  // mysterious extra padding
		// // Try to keep a two-column case balanced.
		// parts_per_column = Math.min(parts_per_column, Math.ceil(this.parts_bin.length / 2));
		for (let i = 0; i < this.parts_bin.length; ++i) {
			const wrapper = document.createElement('div');
			wrapper.style.position = 'relative';
			wrapper.style.height = part_h + 'px';

			wrapper.appendChild(this.parts_bin[i].canvas);
			parts_table.appendChild(wrapper);
		}
	...
	Schematic.prototype.help = function () {
		...
		let strHelp = i18n.strSHelp + i18n.strAddC + i18n.strAddW + i18n.strSel + i18n.strMove + i18n.strDel + i18n.strRot + i18n.strView + i18n.strProp + i18n.strNum;
		//window.confirm(strHelp);
		setTimeout(() =>
            showMessage(strHelp, "info", false),
            0);
	};
	...
	Schematic.prototype.paste = function () {
		...
		for (let i = sch_clipboard.length - 1; i >= 0; --i) {
			let c = sch_clipboard[i];
			var new_c = c.clone(this.cursor_x + (c.x - left) - (this.cursor_x + (c.x - left)) % 48, this.cursor_y + (c.y - top) - (this.cursor_y + (c.y - top)) % 48);
	...
	Schematic.prototype.has_short_circuit = function (convertedNetlist) {
		...
		let list = [];
		let node_groups = [];
		convertedNetlist = convertedNetlist.split(/\r?\n/)
		const visited = new Set();
		for (let line of convertedNetlist) {
			list.push(line.split(" "));
		}
		for (let i in list){
			if(list[i][0] === "W"){
				wires.push(['W'+count,list[i][1],list[i][2]]);
				count++;
			}
			else{
				comps.push([list[i][0],list[i][1],list[i][2]]);
			}
		}
		wires = wires.map(([name, a, b]) => [
			name,
			a,
			b.replace(';', '')
		]);

		function nodesOf(wire) {
			return [wire[1], wire[2]];
		}

		for (const wire of wires) {
			let wireNodes = nodesOf(wire);
			if (visited.has(wire[0])) continue;
			const stack = [wireNodes];
			const group = [];
			const nodeSet = new Set(wireNodes);

			visited.add(wire[0]);

			while (stack.length > 0) {
				const current = stack.pop();
				current.forEach((node) => group.push(node));

				for (const other of wires) {
					if (visited.has(other[0])) continue;

					const [a, b] = nodesOf(other);

					if (nodeSet.has(a) || nodeSet.has(b)) {
						visited.add(other[0]);
						stack.push(nodesOf(other));
			...
			node_groups.push([...new Set(group)]);
		}
		// Check if both component nodes belong to the same node group
		for (let comp of comps) {
			let n1 = comp[1];
			let n2 = comp[2];
			for (let node_group of node_groups){
				for (let i in node_group) {
					if (n1 === node_group[i]){
						for (let j in node_group){
							if (n2 === node_group[j]){
								short_circuit = true;
							}
						}
	...
	Schematic.prototype.has_one_comp = function (convertedNetlist) {
		let bool = true;
		let list = [];
		let compList = [];
		for (let line of convertedNetlist) {
		...
		for (let i in list){
			if (list[i][0].startsWith("R") || list[i][0].startsWith("C") || list[i][0].startsWith("L")){
	...
	Schematic.prototype.save_netlist = function () {
		...
		if (netlist != undefined) {
			this.input.value = JSON.stringify(netlist);
			download(netlist, "simplipfy editor netlist.txt", "text/plain");
		}
	...
	Schematic.prototype.simplipfy_netlist = function () {
		this.label_connection_points();
		var netlist = this.json();
		state.currentEditorElementList = netlist;
		this.input.value = JSON.stringify(netlist);
		...
		if (netlist !== undefined){
			modalXl.show(new SelectModeModal())
		}
		else {
			console.log('No netlist was detected')
			setTimeout(() => { showMessage(i18n.undefined_netlist, 'warning', false) });
		}
	};
			...
			this.dialog(i18n.Open_netlist, content, function (content) {
				...
				if (files.length > 0) {
					...
					reader.onload = function (e) {
						...
						imported_netlist = converter.netlist2Editor(imported_netlist);
	...
	Schematic.prototype.dc_analysis = function () {
		...
		for (let i in comp) {
			if (comp[i].type === 'v' && comp[i].properties['value'].split('(')[0] === 'sin') {
				setTimeout(() => {showMessage(i18n.ckt_alert11, 'warning', false)});
				return;
			}
		}
	...
	Schematic.prototype.setup_ac_analysis = function () {
		...
		if (this.find_probes().length == 0) {
			...
			setTimeout(() => { showMessage(i18n.AC_Analysis_add_a_voltage_probe) });
			return;
		}
	...
	Schematic.prototype.ac_analysis = function (npts, fstart, fstop, ac_source_name) {
		...
		var comp = this.components;
		for (let i in comp) {
			if (comp[i].type === 'v' && comp[i].properties['value'].split('(')[0] === 'dc') {
				setTimeout(() => {showMessage(i18n.ckt_alert9, 'warning', false)});
				return;
			}
		}
		...
		if (typeof results == 'string')
			...
			if (all_max < 1.0e-16) {
				...
				setTimeout(() => { showMessage(i18n.Zero_ac_response) });
			} else {
				for (let i = probes.length - 1; i >= 0; --i) {
					if (probes[i][3] != 'voltage') continue;
					if ((probe_maxv[i] / all_max) < 1.0e-10) {
						...
						setTimeout(() => { showMessage(i18n.Near_zero_ac_response + probe_color[i] + i18n.probe) });
						return;
					}
				}
			}
	...
	Schematic.prototype.transient_analysis = function () {
		...
		if (probes.length == 0) {
			...
			setTimeout(() => { showMessage(i18n.Transient_Analysis_add_a_probe) });
			return;
		}
		...
		this.dialog(i18n.Transient_Analysis, content, function (content) {
			...
			var comp = sch.components;
			for (let i in comp) {
				if (comp[i].type === 'v' && comp[i].properties['value'].split('(')[0] === 'dc') {
					setTimeout(() => {showMessage(i18n.ckt_alert10, 'warning', false)});
					return;
				}
			}
			...
			if (typeof results == 'string')
				...
				for (let i = probes.length - 1; i >= 0; --i) {
					...
					if (v == undefined) {
						...
						setTimeout(() => { showMessage(i18n.The + color + i18n.probe_is_connected_to_node + '"' + label + '"' + i18n.which_is_not_an_actual_circuit_node) });
	...
	Schematic.prototype.redraw_background = function () {
		...
		if (!this.diagram_only && this.show_grid) {
			...
			const grid_step = this.grid * 6;

			for (let i = grid_step * Math.ceil(first_x / grid_step); i < last_x; i += grid_step)
				this.draw_line(c, i, first_y, i, last_y, 1);

			for (let i = grid_step * Math.ceil(first_y / grid_step); i < last_y; i += grid_step)
	...
	Schematic.prototype.redraw = function () {
		...
		if (!this.diagram_only) {
			...
			c.moveTo(x + 2, y - 4);			// start
			c.lineTo(x + 11, y - 4);			// straight right to tip
			c.lineTo(x + 11, y - 13);			// straight up
			c.stroke();
	...
	function schematic_mouse_enter(event) {
		if (!event) event = window.event;
		if (schematic.isDialogOpen) return;
		var sch = event.target.schematic;
	...
	function schematic_mouse_down(event, sch) {
		...
		if (sch.new_part) {
			...
			// make a clone of the component in the parts bin
			part = part.component.clone(Math.floor(sch.cursor_x/48)*48, Math.floor(sch.cursor_y/48)*48);
			part.add(sch);  // add it to schematic
			part.set_select(true);
	...
	function schematic_mouse_move(event, sch) {
		...
		var x = mx / sch.scale + sch.origin_x;
		var y = my / sch.scale + sch.origin_y;
		const sch_grid = sch.grid * 6;
		sch.cursor_x = Math.round(x / sch_grid) * sch_grid;
		sch.cursor_y = Math.round(y / sch_grid) * sch_grid;

		if (sch.wire) {
			if (Math.abs(sch.wire[0] - sch.cursor_x) > Math.abs(sch.wire[1] - sch.cursor_y)) {
				sch.wire[2] = sch.cursor_x;
				sch.wire[3] = sch.wire[1];
			} else {
				sch.wire[2] = sch.wire[0];
	...
	function schematic_double_click(event) {
		...
		for (let i = sch.components.length - 1; i >= 0; --i)
			if (sch.components[i].edit_properties(x, y, sch.components))
				break;
	...
	function build_window(title, content, offset, showDownloadIcon) {
		...
		head.addEventListener('touchstart', window_mouse_down, false);
		...
		win.style.left = win.left + 'vw';
		win.style.top = win.top + 'vh';
	...
	function window_mouse_down(event) {
		...
		document.addEventListener('touchmove', window_mouse_move, false);
		document.addEventListener('touchend', window_mouse_up, false);
		...
		if(event.type === "touchstart"){
			win.drag_x = Math.abs(event.changedTouches[0].pageX);
			win.drag_y = Math.abs(event.changedTouches[0].pageY);
		}
	...
	function window_mouse_move(event) {
		...
		if (win !== undefined && win.drag_x) {
			if(event.type === "touchmove"){
				dx =event.changedTouches[0].pageX - win.drag_x;
				dy =event.changedTouches[0].pageY - win.drag_y;
			}
			else{
				dx = event.pageX - win.drag_x;
				dy = event.pageY - win.drag_y;
			}
	...
	Schematic.prototype.add_tool = function (icon, tip, callback) {
		...
		if (icon.search('data:image') != -1) {
		...
		} else if (icon.search('SimpliPFy') != -1){
            tool = document.createElement('div');
            tool.id = "editor-simplipfy-btn"
            tool.classList.add("circuitStartBtn", "mx-auto")
            tool.style.maxWidth = "100px";
            tool.style.maxHeight = "30px";
            ...
            if (!state.backendReady){
                ...
                <span class="button-text" style="color: ${textColor}">SimpliPFy</span>`
            }
            else {
                tool.style.backgroundColor = `${colors.keyYellow}`
                tool.innerHTML = `<span class="button-text" style="color: ${textColor}">SimpliPFy</span>`
            }
			...
			tool.style.color = icon_style;
			let label = document.createTextNode(icon);
	...
	Schematic.prototype.enable_tool = function (tname, which) {
		...
		if (tool !== undefined) {
			...
			if (!which) {
				tool.style.borderColor = background_style;
				// I don't think this is necessary and it messes with the "new feature" message
				//tool.sch.message('');
			}
		}
	};
	...
	*/
	...
	Component.prototype.edit_properties = function (x, y, allComp) {
		...
		if (this.near(x, y)) {
			...
			// const rulesComponent = /^\d*$/;

			this.sch.dialog(i18n.Edit_Properties, content, function (content) {
				for (let i in content.fields){
					if (i === "name"){
						// if(!rulesComponent.test(fields['name'].value) && fields['name'].value !== ""){
						// 		console.log('Wrong name for resistor!')
						// 		setTimeout(() => { showMessage(i18n.wrong_Name, 'warning', false) });
						// 		return;
						// }
						let sourceCount = 0;
						for (let j in allComp){
							if (allComp[j].properties.name === content.fields['name'].value && fields['name'].value !== c.properties.name &&
								content.fields['name'].value !== '' && allComp[j].type !== 'v'){
								console.log('Duplicated index are not recommended!')
								if(!confirm(i18n.double_index)){
									return;
								}
								break;
							}
							if (allComp[j].type === 'v') sourceCount++;
						}
						for (let j in allComp){
							if (sourceCount > 1 && allComp[j].properties.name === fields['name'].value && allComp[j].type === 'v' && content.fields['name'].value !== ''){
								console.log('Same index not recommended!')
								setTimeout(() => { showMessage(i18n.same_Index, 'warning', false) });
							}
						}
					}
					// if (content.fields[i].value === "0"){
					// 	console.log('Wrong value')
					// 	setTimeout(() => { showMessage(i18n.invalid_Value, 'warning', false) });
					// 	return;
					// }
	...
	ConnectionPoint.prototype.propagate_label = function (label) {
		...
		if (this.label === undefined) {
			...
			setTimeout(() => { showMessage(i18n.Node_has_two_conflicting_labels + this.label + ', ' + label) });
	};
	...
	Resistor.prototype.draw = function (c) {
		...
		if (this.properties.r)
			this.draw_text(c, this.properties.r + '\u03A9', 8, 30, 3, property_size);
		if (this.properties.name)
			this.draw_text(c, "R"+this.properties.name, -8, 18, 5, property_size);
	};
	...
	Capacitor.prototype.draw = function (c) {
		...
		if (this.properties.c)
			this.draw_text(c, this.properties.c + 'F', 12, 30, 3, property_size);
		if (this.properties.name)
			this.draw_text(c, "C"+this.properties.name, -12, 18, 5, property_size);
	};
	...
	Inductor.prototype.draw = function (c) {
		...
		if (this.properties.l)
			this.draw_text(c, this.properties.l + 'H', 8, 30, 3, property_size);
		if (this.properties.name)
			this.draw_text(c, "L"+this.properties.name, -8, 18, 5, property_size);
	};
	...
	Source.prototype.draw = function (c) {
		...
		if (this.properties.name)
			this.draw_text(c, "V"+this.properties.name, -16, 18, 5, property_size);
		if (this.properties.v) {
			//this.draw_text(c,this.properties.v,16,24,3,property_size);
			let index = this.properties.v.indexOf('(');
			let fun = i18n[this.properties.v.slice(0, index)];   // translation of function name before the "("
			let params = this.properties.v.slice(index);   		// parameters after the "("
			this.draw_text(c, fun + params, 14, 30, 3, property_size);
		}
	};
	...
	Source.prototype.build_content = function (src) {
		...
		if (src == undefined) {
			fields.value = this.properties.v;
		} else {
	...
	Source.prototype.edit_properties = function (x, y, allComp) {
		if (this.near(x, y)) {
			this.src = undefined;
			if (typeof cktsim != 'undefined')
				this.src = cktsim.parse_source(this.properties.v);
			var content = this.build_content(this.src);

			this.sch.dialog(i18n.Edit_Properties, content, function (content) {
				var c = content.component;
				var fields = content.fields;
				// const rulesSourceComponent = /^[a-z]*$|^\d*$/;
				var first = true;
				let sourceCount = 0;
				var value = '';
				for (let label in fields) {
					if (label === 'name') {
						// check if index syntax is correct
						// if (!rulesSourceComponent.test(content.fields['name'].value)) {
						// 	console.log('Wrong name!')
						// 	setTimeout(() => { showMessage(i18n.wrong_Name_V, 'warning', false) });
						// 	return;
						// }
						for (let j in allComp){
							if (allComp[j].properties.name === fields['name'].value && fields['name'].value !== c.properties.name && content.fields['name'].value !== '' && allComp.type === 'v'){
								console.log('Duplicated name not allowed!')
								setTimeout(() => { showMessage(i18n.double_name + content.fields['name'].value, 'warning', false) });
								return;
							}
							if (allComp[j].type === 'v') sourceCount++;
						}
						for (let j in allComp){
							if (sourceCount > 1 && allComp[j].properties.name === fields['name'].value && allComp[j].type !== 'v'){
								console.log('Same index not recommended!')
								setTimeout(() => { showMessage(i18n.same_Index, 'warning', false) });
							}
						}
	...
	VSource.prototype.clone = function (x, y) {
		return new VSource(x, y, this.rotation, this.properties.name, this.properties.v);
	};
	...
	ISource.prototype.clone = function (x, y) {
		return new ISource(x, y, this.rotation, this.properties.name, this.properties.v);
	};
...
///////////////////////////////////////////////////////////////////////////////
//
//	Custom netlist converter for Simplify
//
///////////////////////////////////////////////////////////////////////////////

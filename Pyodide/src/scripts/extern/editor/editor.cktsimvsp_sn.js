...
/*jshint esversion: 6 */
/** @type {typeof editorEnTexts.editorStrings} */
let i18n = window.english.editorPage.editorStrings;	//internationalization translated strings
...
cktsim = (function() {
	...
	Circuit.prototype.finalize = function() {
		...
		if (n_vsrc == 0) {
			...
			setTimeout(() => {showMessage(i18n.ckt_alert1, 'warning', false)});
			setTimeout(() => {showMessage(i18n.ckt_alert2, 'warning', false)});
			return false;		
		    }
	...
	Circuit.prototype.load_netlist = function(netlist) {
		...
		var name = type.toUpperCase() + properties['name'];
		...
		setTimeout(() => {showMessage(i18n.ckt_alert3, 'warning', false)});
		return false;
	    }
	    return true;
	...
	Circuit.prototype.dc = function() {
		...
		if (this.current_sources.length > 0) {
			...
			setTimeout(() => {showMessage(i18n.ckt_alert4, 'warning', false)});
		} else {
		    //alert('Newton Method Failed, it may be your circuit or it may be our simulator.');
			setTimeout(() => {showMessage(i18n.ckt_alert5, 'warning', false)});
		}
	...
	// Transient analysis (needs work!)
		...
		if (this.dc() == undefined) { // DC fail, realloc mats, vects.
			...
			setTimeout(() => {showMessage(i18n.ckt_alert6, 'warning', false)});
    ...
    Circuit.prototype.ac = function(npts,fstart,fstop,source_name) {
	    ...
	    if (this.device_map[source_name] === undefined) {
			...
			setTimeout(() => {showMessage(i18n.ckt_alert7 + source_name, 'warning', false)});
		...
		if (this.device_map[name] === undefined) 
			...
			setTimeout(() => {showMessage(i18n.ckt_warning1, 'warning', false)});
		    this.device_map[name] = d;
		}
	    }
	    return d;
   ...
   Circuit.prototype.opamp2 = function(nInP,nInN,nO,nP,nN,Gain,Ro,Rin,name) {
	...
	///////////////////////////////////////////////////////////////////////////////
	//
	//  Simple Voltage-Controlled Voltage Source Op Amp model - ideal
	//
	///////////////////////////////////////////////////////////////////////////////
	...
	///////////////////////////////////////////////////////////////////////////////
	//
	//  Simple Voltage-Controlled Voltage Source Op Amp model - realistic
	//
	///////////////////////////////////////////////////////////////////////////////

from typing import Union

import sympy as sp

import lcapyInskale
from lcapyInskale import Circuit, j, omega0 as lcapy_omega0, state
from simplipfy.Helpers.netlistLine import NetlistLine
from simplipfy.Svg.drawingConfig import drawing_config_instance as dc


def ComponentToImpedance(netlistLine: str,
                         omega_0: Union[float, str] = None,
                         skipElementTypes=None,
                         replaceElementType=None,
                         replaceValueWith=None,
                         debug=False,
                         newLine=False
                         ) -> str:
    """
    :param netlistLine: a line of a lcapy.Circuit.netlist()
    :param omega_0: circular frequency used for the conversion to impedance, If None symbol lcapy.omega_0 is used.
    :param skipElementTypes: List of element types that should not be converted to impedance,
     if None ["V", "W"] is used
    :param replaceElementType: dictionary with element types as keys and the new element type as value,
     if None {"R": "Z", "L": "Z", "C": "Z"} is used
    :param replaceValueWith: dictionary with element types as keys and the new value as value,
     if None {"R": "value", "L": "j*value*omega_0", "C": "-j/(value*omega_0)"} is used

    Converts the value of a NetlistLine to impedance. Extracts the value from the NetlistLine.value property replaces it
    with the according impedance value and replaces the type with the according impedance type.
    Then reconstructs the new NetlistLine and returns the netlist as a string.

    E.g.:
    converts: C2 5 6 {100}; C2 5 6 100; down;
    to: Z2 5 6 {-j/(100*omega_0)}; down

    ::

        skipElementTypes = ["V", "W"]
        replaceElementType = {"R": "Z", "L": "Z", "C": "Z"}
        replaceValueWith = {"R": "value", "L": "j*value*omega_0", "C": "-j/(value*omega_0)"}
    """
    if skipElementTypes is None:
        skipElementTypes = ["V", "W"]
    if replaceElementType is None:
        replaceElementType = {"R": "Z", "L": "Z", "C": "Z"}
    if replaceValueWith is None:
        replaceValueWith = {"R": "value", "L": "j*value*omega_0", "C": "-j/(value*omega_0)"}

    netLine = NetlistLine(netlistLine)

    if isinstance(omega_0, float):
        _omega_0 = omega_0
    elif isinstance(omega_0, str):
        _omega_0 = sp.parse_expr(omega_0, local_dict={"pi": sp.pi})
    else:
        _omega_0 = lcapy_omega0

    if netLine.type in skipElementTypes:
        returnVal = netLine.line
    elif netLine.type in list(replaceElementType.keys()):
        state.show_units = False

        expr = replaceValueWith[netLine.type]
        parsedVal = sp.parse_expr(netLine.value)
        parsedEx = sp.parse_expr(expr, {"value": parsedVal, "omega_0": _omega_0, "j": j})
        netLine.value = '{'+str(parsedEx)+'}'

        netLine.type = replaceElementType[netLine.type]
        returnVal = netLine.reconstruct()
    else:
        returnVal = netLine.line

    if debug:
        print(f"{netlistLine} -> {returnVal}")

    if newLine:
        return returnVal + "\n"
    else:
        return returnVal


def ImpedanceToComponent(strNetlistLine: str = None,
                         netlistLine: NetlistLine = None,
                         omega_0: Union[float, str] = None) -> str:
    """
    :param strNetlistLine: a line of a lcapy.Circuit.netlist()
    :param netlistLine: simplipfy.Helpers.netlistLine.NetlistLine() Object, created from strNetlistLine
    :param omega_0: circular frequency used for the conversion to impedance, If None symbol lcapy.omega_0 is used.
    :returns: The converted netlist line as a string with the transformed value and type

    Converts a NetlistLine to its corresponding Component (R, L, or C) if it is not possible it returns the input strNetlistLine
    """

    if netlistLine:
        netLine = netlistLine
    elif strNetlistLine:
        netLine = NetlistLine(strNetlistLine)
    else:
        raise AttributeError("strNetlistLine or netlistLine need a value")

    netLine.value, netLine.type = ValueToComponent(netLine.value, omega_0=omega_0)

    return netLine.reconstruct()


def ValueToComponent(value, omega_0: Union[float, str] = None) -> (sp.Mul, str):
    """
    :param value: the value to convert
    :param omega_0: circular frequency used for the conversion to impedance, If None symbol lcapy.omega_0 is used.
    :returns: The converted value and the type of the component

    Converts an impedance value to its corresponding component value
    """
    if isinstance(omega_0, float):
        _omega_0 = omega_0

    # omega_0, omega0 and omega are already existing symbols and thus create an unintended behavior
    elif isinstance(omega_0, str) and omega_0 not in ["omega_0", "omega0", "omega"]:
        _omega_0 = sp.parse_expr(omega_0, local_dict={'pi': sp.pi})
        if isinstance(_omega_0, sp.Symbol):
            _omega_0 = sp.Symbol(omega_0, real=True, nonzero=True, finite=True)

    else:
        _omega_0 = lcapy_omega0

    # if it comes in as lcapy.Expr object it has a unit which will be in the str and should not be in there
    if not isinstance(value, str):
        if isinstance(value, lcapyInskale.Expr):
            value = str(value.expr)
        else:
            value = str(value)

    _value = sp.parse_expr(value, local_dict={'omega_0': _omega_0, 'j': j, 'pi': sp.pi})
    if _value == sp.zoo and _omega_0 == 0:
        return _value, "C"
    # if there is a resistor with 0 ohms and omega_0 is also 0 this gives back an inductor instead of a resistor
    elif _value == 0 and _omega_0 == 0:
        return _value, "L"

    freeSymbols = _value.free_symbols - {_omega_0, 'pi'}

    # this code broke when the calculation is done symbolically e.g. with R1 as value for resistor R1
    # I think that was an unnecessary check based on the fact hat without symbolic calculations there never could be
    # more than one free symbol but is not necessary for this function to work correctly
    # if len(freeSymbols) > 1:
    #    raise AttributeError(f"Too many free symbols in {_value}, free symbols: {freeSymbols}")

    if len(freeSymbols) > 1:
        print(f"More than one free symbol in {value} excluding j, I, omega_0 and pi, assuming this this is a symbolic calculation")
        return value, "Z"
    elif len(freeSymbols) == 1:
        freeSymbol = freeSymbols.pop()
        sub_dict = {str(freeSymbol): sp.Symbol(str(freeSymbol), real=True, finite=True, positive=True)}
    else:
        # No free symbols means that the value is a constant
        sub_dict = {}

    # using the sp.im(_value) funktion equals _value/j
    _value = _value.subs(sub_dict).evalf()
    if sp.re(_value).is_zero:
        if not sp.im(_value).is_negative and not sp.im(_value).is_zero:
            returnVal = sp.im(_value) / _omega_0
            returnType = "L"
        elif sp.im(_value).is_negative:
            returnVal = -1 / (sp.im(_value) * _omega_0)
            returnType = "C"
        else:
            returnVal = value
            returnType = "Z"
    elif sp.im(_value).is_zero:
        returnVal = sp.re(_value)
        returnType = "R"
    else:
        returnVal = value
        returnType = "Z"

    return returnVal, returnType


def FileToImpedance(filename: str) -> str:
    """
    :param filename: filename to open, with path and extension
    :returns: converted netlist as str
    
    Converts a netlist file that has a mixture of R L C elements to Z (impedance)
    """
    netlistString = open(filename, "r").read()
    netlist = netlistString.split("\n")

    cct = Circuit(netlistString)

    omega_0 = getOmegaFromCircuit(cct)

    dc.setToDefault()
    conv_netlist = ""
    for line in netlist:
        if line.startswith('#'):
            dc.setOptions(line)
            continue
        conv_netlist += ComponentToImpedance(line, newLine=True, omega_0=omega_0)

    return conv_netlist


def NeedsConversion(netlist: str, checkForTypes=None) -> bool:
    """
    :param netlist: netlist as a list of strings, lines separated by \n
    :param checkForTypes: list of types to check for, if None ["R", "L", "C"] is used
    :returns: True if there is more than one type of component in the netlist, False otherwise

    Checks if there is more than one type of component in the netlist/circuit. Checks for R, L, and C by default
    """
    # define types that are relevant
    if checkForTypes is None:
        checkForTypes = ["R", "L", "C"]

    # create a variable for each relevant type
    hasType = {}
    for elType in checkForTypes:
        hasType[elType] = False

    for line in netlist.splitlines():
        netLine = NetlistLine(line)
        if netLine.type in checkForTypes and not hasType[netLine.type]:
            hasType[netLine.type] = True

    return bool(sum(hasType.values())-1)


def getOmegaFromCircuit(circuit: 'Circuit') -> Union[float, sp.Mul, None]:
    """
    :param circuit: lcapyInskale.Circuit object
    :returns: If omega_0 is a float returns a float, if it is a string returns a sympy expression in any other case
    :warns: If there are multiple sources in the circuit, a warning is raised and the first source is used
     returns 0

    Retrieves omega_0 from the circuit.
    """
    sources = circuit.sources
    if len(circuit.sources) > 1:
        from warnings import warn
        warn("Circuit has more than one source, omega_0 is equivalent to first source")

    if len(circuit.elements[sources[0]].args) >= 3:
        value = circuit.elements[sources[0]].args[2]
        if isinstance(value, str):
            try:
                omega_0 = float(value)
            except ValueError:
                omega_0 = value
            return omega_0
        else:
            return 0

    else:
        return 0

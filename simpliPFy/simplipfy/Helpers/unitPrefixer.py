...
class SIUnitPrefixer:
    ...
    def _findExponentFloatInt(value: Union[float, int]) -> int:
        ...
        :returns: exponent of the value

        Determine the exponent of a float or int value
        """
    ...
    def _findExponentMul(value: lcapyInskale.Expr, forImag=True) -> int:
        """
        :param value: lcapyInskale.Expr to determine the prefix for
        :param forImag, if imag is True the prefix is also determined for expressions that only have an imaginary part
         for an expression with imaginary part and real part 0 is returned
        :returns: exponent of the value
    ...
    def _findSIPrefix(self, exponent: int) -> Prefix:
        """
    ...
    def getSIPrefix(self, value: Union[float, int, Mul, lcapyInskale.Expr]) -> Prefix:
        """
        :param value to get the prefix for
        :returns: the sympy.prefix for the given value

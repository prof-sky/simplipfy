...
with schemdraw.Drawing() as d:
    ...
    d.push()
    elm.Capacitor(id_='C1').right().label('-j3Ω').dot()
    elm.Inductor(id_='L2').down().label('j2Ω').dot().hold()
    elm.Resistor(id_='R1').right().label('5Ω').dot()
    V1 = elm.SourceV().down().reverse().label('5∠-90° V', loc='bot')
    elm.Line().tox(I1.start)
    d.pop()
    elm.Line().up(d.unit*.8)
    L1 = elm.Inductor(id_='L1').tox(V1.start).label('j3Ω')

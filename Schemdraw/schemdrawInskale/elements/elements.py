...
class Element:
    ...
    def label(self,
        ...
        ''' Add a label to the Element.
            ...
            Args:
                ...
                id_: identification string to identify svg-tag
        '''
        if not rotate:
            rotate = 0
    ...
    def _place_label(self, label: Label, theta: float = 0) -> None:
        ...
        label = Label(label.label, label.loc, label.ofst, label.halign,
                      ...
                      label.font, label.mathfont, label.color, label.class_, label.id_)
        ...
        userparams = {}
        userparams['class_'] = label.class_ if label.class_ else self._userparams.get('class_', 'na')
        userparams['id_'] = label.id_ if label.id_ else self._userparams.get('id_', 'na')
        ...
        if isinstance(label.label, (list, tuple)):
            if label.loc == 'top':
                ...
                for i, lbltxt in enumerate(label.label):
                    ...
                    self.segments.append(SegmentText(xy+label.ofst, lbltxt, **segment_params, userparams=userparams))
            elif label.loc == 'bottom':
                xdiv = (xmax-xmin)/(len(label.label)+1)
                for i, lbltxt in enumerate(label.label):
            ...
            self.segments.append(SegmentText(xy+label.ofst, label.label, **segment_params, userparams=userparams))
    ...
    def _draw(self, fig) -> None:
        ...
        if self.params.get('elmbbox', False):
            ...
            Segment(((bbox.xmin, bbox.ymin), (bbox.xmax, bbox.ymin),
                     ...
                     color='blue', lw=.5, userparams=self._userparams).draw(fig, self.transform)
